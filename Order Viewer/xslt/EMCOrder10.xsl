...
<!-- $Id: ptsiiiorder.xsl,v 1.10 2011/06/01 08:00:47 jmigchie Exp $ -->
<!--
     version 1.2  21 August 2011
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:variable name="ORDER">
		<!-- Possible values for stage/@step:
             RSVP | CU | S5 | S100 | S200 | S250 | S300 | P100 | Q300 | S350 | F300 |
		...
		<xsl:choose>
			...
			<xsl:when test="//stage[@step='S250' or @step='S250RESUPPLY']">
...
<!--				<xsl:call-template name="insert-xml-code"/> -->
	...
	<xsl:template match="order">
				...
				<xsl:when test="item-info">
					<xsl:choose>
						...
						<xsl:otherwise>
							...
							<xsl:text>EMC - COVERSHEET</xsl:text>
				...
				<xsl:when test="issue-info">
					<xsl:choose>
						<xsl:when test="$ORDER='ISSUE'">
							<xsl:text>EMC - ISSUE PAGINATION SHEET</xsl:text>
						</xsl:when>
		...
		<p>
			<b><font style="color:#6A5ACD;">
			<xsl:text>MAJ Information:</xsl:text>
			</font></b>
		</p>
...
<!-- EMC Tags Start -->
				...
				<tr>
					<td>
						<xsl:text>Tratie Code:</xsl:text>
					</td>
					<td>
						<xsl:apply-templates select="tra-code"/>
					</td>
				</tr>

				<tr>
					<td>
						<xsl:text>Tratie Racine:</xsl:text>
					</td>
					<td>
						<xsl:apply-templates select="tra-racine"/>
					</td>
				</tr>
				<tr>
					<td>
						<xsl:text>Tratie Title:</xsl:text>
					</td>
					<td>
						<xsl:apply-templates select="tra-title"/>
					</td>
				</tr>
				<tr>
					<td>
						<xsl:text>MAJ No:</xsl:text>
					</td>
					<td>
						<xsl:apply-templates select="maj-info/maj-no"/>
					</td>
				</tr>
				<tr>
					<td>
						<xsl:text>MAJ Cote:</xsl:text>
					</td>
					<td>
						<xsl:apply-templates select="maj-info/maj-cote"/>
					</td>
				</tr>
				<tr>
					<td>
						<xsl:text>MAJ Year:</xsl:text>
					</td>
					<td>
						<xsl:apply-templates select="maj-info/maj-anne"/>
					</td>
				</tr>
...
<!-- EMC Tags End -->
	...
	<xsl:template match="item-info">
		<!-- Table with part of item information. Layout:
            ...
            doi                      no-phys-figs
            pit                      no-bw-figs
            prd-type-as-sent         no-colour-figs
            version nr               no-web-colour-figs
            copyright status         no-e-components
            copyright received date  batch
            embargo                  refers-to-document
            embargo-until-stage      
            righthand start          e-submission-item-nr
            online publication date  eo-item-nr
            online version           editor
            corrections              received date
            offprint payment         revised date
                                     accept date
        -->
<p>
			<b><font style="color:#6A5ACD;">
				...
				<tr>
					<td width="50%">
						<table width="100%">
							...
							<tr>
								<td>
									<xsl:text>Fasicle No:</xsl:text>
								</td>
								<td>
									<xsl:apply-templates select="fas-numro"/>
								</td>
							</tr>
							<tr>
								<td>
									<xsl:text>Fasicle Vol:</xsl:text>
								</td>
								<td>
									<xsl:apply-templates select="fas-volume"/>
								</td>
							</tr>
							...
							<tr>
								<td>
									<xsl:text>Typesetting model:</xsl:text>
								</td>
								<td>
									<xsl:apply-templates select="typesetting-model"/>
								</td>
							</tr>

							<tr>
								<td>
									<xsl:text>Typesetting model format:</xsl:text>
								</td>
								<td>
									<xsl:apply-templates select="typesetting-model-format"/>
								</td>
							</tr>
							<tr>
								<td>
...
<!--							<tr>
					...
					<td width="50%">
						<table width="100%">
							...
							<tr>
								<td>
									<xsl:text>Nr of physical figures:</xsl:text>
								</td>
								<td>
									<b><xsl:apply-templates select="no-phys-figs"/></b>
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									<b><xsl:apply-templates select="no-web-colour-figs"/></b>
								</td>
							</tr>
							<tr>
								<td>
...
<!--							<tr>
		...
		<xsl:call-template name="insert-hrule"/>
		<p>
			<!-- Another part of item information -->
			<table width="100%">
				<tr>
					<td>
...
<!--				<tr>
				...
				<tr>
					...
					<td>
						...
						<tr>
						<xsl:if test="e-components/no-arbre-decisionnels-interactifs">
						<td><xsl:text>Arbre: </xsl:text></td>
						<td><xsl:apply-templates select="e-components/no-arbre-decisionnels-interactifs"/></td>
						</xsl:if></tr>
						<tr>
						<xsl:if test="e-components/no-iconos-supp">
						<td><xsl:text>Iconos-Supp: </xsl:text></td>
						<td><xsl:apply-templates select="e-components/no-iconos-supp"/></td>
						</xsl:if></tr>
						<tr>
						<xsl:if test="e-components/no-videos-animation">
						<td><xsl:text>Video: </xsl:text></td>
						<td><xsl:apply-templates select="e-components/no-videos-animation"/></td>
						</xsl:if></tr>
						<tr>
						<xsl:if test="e-components/no-documents-legaux">
						<td><xsl:text>Doc-leg: </xsl:text></td>
						<td><xsl:apply-templates select="e-components/no-documents-legaux"/></td>
						</xsl:if></tr>
						<tr>
						<xsl:if test="e-components/no-info-patient">
						<td><xsl:text>Info-patient: </xsl:text></td>
						<td><xsl:apply-templates select="e-components/no-info-patient"/></td>
						</xsl:if></tr>
						<tr>
						<xsl:if test="e-components/no-info-supp">
						<td><xsl:text>Info-Supp: </xsl:text></td>
						<td><xsl:apply-templates select="e-components/no-info-supp"/></td>
						</xsl:if></tr>
						<tr>
						<xsl:if test="e-components/no-auto-evaluations">
						<td><xsl:text>Autoevaluation: </xsl:text></td>
						<td><xsl:apply-templates select="e-components/no-auto-evaluations"/></td>
						</xsl:if></tr>
						<tr>
						<xsl:if test="e-components/no-cas-cliniques">
						<td><xsl:text>Cas-clinique: </xsl:text></td>
						<td><xsl:apply-templates select="e-components/no-cas-cliniques"/></td>
						</xsl:if></tr>
						<tr>
						<xsl:if test="e-components/no-quotidien">
						<td><xsl:text>Quotidien: </xsl:text></td>
						<td><xsl:apply-templates select="e-components/no-quotidien"/></td>
						</xsl:if></tr>
...
<!--				<tr>
	...
	<xsl:template match="item-info" mode="abp">
		<!-- Layout:
			...
			Effect cover date        Paper type cover
			Volume/Issue             Trimmed size
			S250 sequence nr         Head margin
			Pages                    Back margin
			                         Typeset model
	...
	<xsl:template match="issue-info">
		<!-- Table with part of issue information. Layout:
            ...
            jid                        journal title
            volume/issue               journal nr
            pii                        issn
            version nr                 cover info
            effect cover date          special issue
            cover date printed         
            zero warehousing *         prelim pages
            article based publishing   interior pages
            embargo                    extra pages
            issue production type      insert pages
            buffer status              backmatter pages
            hold-until-date            nr pages print
                                       nr pages web
            paper type interior        nr pages total
            paper type cover           nr pages blank
            cover finishing            nr pages adverts
            cover print type           typeset model
            print type                 right-hand start
            binding type               corrections
            offprint type
            trimmed size
            head margin
            back margin
		...
		<p>
			<b><font style="color:#6A5ACD;">
			<xsl:text>Issue Information:</xsl:text>
			</font></b>
		</p>
					...
					<td width="50%">
						<table width="100%">
							...
							<tr>
								...
								<td>
									<xsl:apply-templates select="general-info/doi"/>
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									<xsl:choose>
										<xsl:when test="general-info/article-based-publishing">
											<xsl:apply-templates
												select="general-info/article-based-publishing"/>
										</xsl:when>
										<xsl:otherwise>
						...
						<table width="100%">
							...
							<tr>
								...
								<td>
									<a href="#specialissue">
										<xsl:apply-templates
											select="general-info/special-issue/special-issue-id"/>
									</a>
								</td>
							</tr>
							<tr>
...
<!--							<tr>
								...
								<td>
									<xsl:call-template name="insert-pages-from-rows">
										<xsl:with-param name="rows"
											select="issue-content/row[prefix='EX']"/>
							...
							<tr>
								...
								<td>
									<xsl:call-template name="insert-pages-from-rows">
										<xsl:with-param name="rows"
											select="issue-content/row[prefix='IN']"/>
...
<!--							<tr>
	...
	<xsl:template match="issue-content" mode="ISSUE">
		<!-- table, 23 columns, content from issue-content/row -->
		<p>
			<table width="100%">
				<tr>
					<th>
						<xsl:text>Article Code</xsl:text>
					</th>
					<!--th>
						<xsl:text>PII</xsl:text>
					</th
...
<!--					<th>
					...
					<th>
						<xsl:text>Model format</xsl:text>
					</th>
					<th>
...
<!--					<th>
					...
					<th>
						<xsl:text>Offpr. paym. recd</xsl:text>
					</th> -->
					<th>
	...
	<xsl:template match="issue-content" mode="OFFPRINTS">
		<!-- table, 19 columns, content from issue-content/row -->
		<p>
			<table width="100%">
				<tr>
					<th>
	...
	<xsl:template match="issue-content" mode="PRINT-BIND">
		<!-- table, 14 columns, content from issue-content/row -->
		<p>
			<table width="100%">
				<tr>
					<th>
	...
	<xsl:template match="row" mode="ISSUE">
		<!-- tr, 23 columns, empty td's depending on row/@type -->
		<xsl:choose>
...
<!--					<td>
	...
	<xsl:template match="row" mode="OFFPRINTS">
		<!-- tr, 19 columns, empty td's depending on row/@type -->
		<xsl:choose>
			<xsl:when test="@type='ce'">
				<tr>
					<td>
	...
	<xsl:template match="row" mode="PRINT-BIND">
		<!-- tr, 14 columns, empty td's depending on row/@type -->
		<xsl:choose>
					...
					<td width="50%">
						<table width="100%">
							<tr>
								...
								<td>
									<xsl:apply-templates
										select="following-sibling::additional-issue-info/paper-type-interior"
									/>
								</td>
							</tr>
							<tr>
								...
								<td>
									<xsl:apply-templates
										select="following-sibling::additional-issue-info/paper-type-cover"
									/>
								</td>
							</tr>
							<tr>
								...
								<td>
									<xsl:apply-templates
										select="following-sibling::additional-issue-info/trimmed-size"
									/>
								</td>
							</tr>
							<tr>
								...
								<td>
									<xsl:apply-templates
										select="following-sibling::additional-issue-info/head-margin"
									/>
								</td>
							</tr>
							<tr>
								...
								<td>
									<xsl:apply-templates
										select="following-sibling::additional-issue-info/back-margin"
									/>
								</td>
							</tr>
							<tr>
								...
								<td>
									<xsl:apply-templates
										select="following-sibling::additional-issue-info/typeset-model"
									/>
								</td>
							</tr>
						</table>
	...
	<xsl:template match="issue-remark">
		<p>
			...
			<xsl:apply-templates select="remark|diff/remark"/>
	...
	<xsl:template match="issue-remarks">
		...
		<xsl:apply-templates
			select="issue-remark[remark-type!='Advert details' and remark-type!='ADV']"/>
	</xsl:template>
...
<xsl:text>
	...
	<xsl:template name="insert-xml-code">
		<div id=""
			style="background-color:#D7FFCF; position: relative; left:0.3%; border-style:solid; border-width:1; padding:5">
			<span style="font-size:70%">XML code</span>
			<br/>
			<span id="xmlcode" onclick="expColl(part0,expanded0,this)"
				style="color:red; font-weight:900; font-size:150%; font-family:courier"
		...
		<xsl:if test="$no-pages &gt; 0">
			...
			<xsl:for-each select="$rows">
				...
				<xsl:variable name="pf">
					<xsl:value-of
						select="translate(page-from,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz','')"
					/>
				</xsl:variable>
				<xsl:variable name="pt">
					<xsl:value-of
						select="translate(page-to,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz','')"
					/>
				</xsl:variable>
				<xsl:variable name="nextpage">
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:value-of
								select="translate($rows[$pos + 1]/page-from,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz','')"
							/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
						...
						<xsl:otherwise>
							<xsl:choose>
								<xsl:when test="$rows[$pos - 1]/page-to">
									<xsl:value-of
										select="translate($rows[$pos - 1]/page-to,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz','')"
									/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of
				...
				<xsl:choose>
					...
					<xsl:when test="$pos!=1 and $pos!=last()">
						...
						<xsl:if
							test="$pf = $prevpage + 1 and page-to and $pf=$pt and $pf != $nextpage - 1">
							<xsl:value-of select="concat($ENDASH,page-from)"/>
						</xsl:if>
	...
	<xsl:template name="insert-advert-details">
		...
		<xsl:apply-templates
			select="issue-remarks/issue-remark[remark-type='Advert details' or remark-type='ADV']"/>
	</xsl:template>
    ...
    What if an aff contains all child elements but one?
    (Actually, no cty, no zipcode, but a cny.)
    
    Solve colour-fig-nr-print/colour-fig-nr-print_ROW[@num='1'] problem
    I believe it is no problem at all, as the developers use the same stylesheet _in_ PTS

    Italic tr's via style?
    
    What to do with issue-labels-info?
