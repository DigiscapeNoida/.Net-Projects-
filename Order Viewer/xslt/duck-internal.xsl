...
<xsl:otherwise><xsl:apply-templates select="//item-info/pii"/></xsl:otherwise>
           ...
           <tr><td>Article No:</td><td><xsl:choose>
<xsl:when test="//item-info/different/article-number"><font color="red"><b><u><xsl:apply-templates select="//item-info/different/article-number"/></u></b></font></xsl:when>
<xsl:when test="//item-info/delete/article-number"><font color="blue"><b><s><u><xsl:apply-templates select="//item-info/delete/article-number"/></u></s></b></font></xsl:when> 
<xsl:when test="//item-info/insert/article-number"><font color="green"><b><u><xsl:apply-templates select="//item-info/insert/article-number"/></u></b></font></xsl:when> 
<xsl:otherwise><xsl:apply-templates select="//item-info/article-number"/></xsl:otherwise>
</xsl:choose></td></tr>
...
<xsl:otherwise><xsl:apply-templates select="//corr-author//aff//ead"/></xsl:otherwise>
  ...
  <xsl:template match="item-remarks">
	 ...
	 <td width="5%"> <xsl:choose>
<xsl:when test="different/remark-type"><font color="red"><b><u>[ <xsl:value-of select="different/remark-type"/>]</u>&#32;&#32;</b></font></xsl:when>
<xsl:when test="delete/remark-type"><font color="blue"><b><s><u>[<xsl:value-of select="delete/remark-type"/>]</u></s>&#32;&#32;</b></font></xsl:when> 
<xsl:when test="insert/remark-type"><font color="green"><b><u> [<xsl:value-of select="insert/remark-type"/>]</u>&#32;&#32;</b></font></xsl:when> 
<xsl:when test="remark-type"><b>[<xsl:value-of select="remark-type"/>]&#32;&#32;</b></xsl:when>
</xsl:choose>      
       <xsl:choose>
<xsl:when test="diff/remark!=''"><td width="40%"><font color="red"><xsl:value-of select="diff/remark"/></font></td></xsl:when><xsl:otherwise><td width="40%"><xsl:value-of select="remark"/></td></xsl:otherwise>
       ...
       <td width="40%"> 
        <xsl:choose>
       <xsl:when test="diff/remark!=''"><font color="blue"><xsl:value-of select="response"/></font></xsl:when><xsl:otherwise><xsl:value-of select="response"/></xsl:otherwise>
       </xsl:choose>
</td>
     </tr>
     </xsl:for-each>
