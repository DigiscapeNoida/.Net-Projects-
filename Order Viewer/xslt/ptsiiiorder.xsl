...
<!--
     version 1.7r3  15 August 2013
     Copyright (c) 2011-2013 Elsevier B.V.
-->
<!--
    v1.7r3  15 August 2013  [PTS release 2013.3]
    Changes with respect to v1.7r2:
    - Displaying element employer in item-info

    v1.7r2  14 August 2013  [PTS release 2013.3]
    Changes with respect to v1.7r1:
    - Displaying element license (renamed)
    - Not displaying element employer

    v1.7r1  13 August 2013  [PTS release 2013.3]
    Changes with respect to v1.6:
    - Changed template insert-refers-to-document
    - Displaying new elements paid-ads, copyright-license and employer
...
<!--				<tr>
	...
	<xsl:template match="item-info">
		<!-- Table with part of item information. Layout:
            ...
            pii                      no-mns-pages
            doi                      no-phys-figs
            article-number           no-bw-figs
            pit                      no-colour-figs
            prd-type-as-sent         no-web-colour-figs
            version nr               no-e-components
            language                 batch
            CrossMark                refers-to-document
            copyright status
            employer
            copyright received date  e-submission-item-nr
            license                  eo-item-nr
            embargo                  editor
            embargo-until-stage      received date
            righthand start          revised date
            online publication date  accept date
            online version           cpc
            corrections              in-scope-cpc
            corrections-uri
            revised-proof
            offprint payment
        -->
                    ...
                    <td>
                        <xsl:apply-templates select="article-number"/>
							...
							<tr>
								...
								<td>
									<b><xsl:apply-templates select="no-colour-figs"/></b>
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									<b><xsl:apply-templates select="language"/></b>
								</td>
							</tr>
							<tr>
								<td>
	...
	<xsl:template match="issue-info">
		<!-- Table with part of issue information. Layout:
            ...
            pii                        issn
            version nr                 isbn
            effect cover date          cover info
            cover date printed         special issue
            zero warehousing *         
            article based publishing   prelim pages
            embargo                    interior pages
            issue production type      extra pages
            buffer status              insert pages
            hold-until-date            backmatter pages
                                       nr pages print
            paper type interior        nr pages web
            paper type cover           nr pages total
            cover finishing            nr pages blank
            cover print type           nr pages adverts
            print type                 typeset model
            binding type               right-hand start
            offprint type              paid-ads
            trimmed size               corrections
            head margin
            back margin
            issue weight
            spine width
	...
	<xsl:template match="issue-content" mode="ISSUE">
		<!-- table, 25 columns, content from issue-content/row -->
		<p>
			<table width="100%">
				<tr>
					<th>
	...
	<xsl:template match="row" mode="ISSUE">
		<!-- tr, 24 columns, empty td's depending on row/@type -->
		<xsl:choose>
	...
	<xsl:template match="corr-author" mode="name-only">
		...
		<xsl:apply-templates select="snm"/>
        <xsl:if test="orcid">
        <!--<xsl:if test="orcid and $ORDER!='PRINT-BIND'">-->
            <xsl:value-of select="' '"/>
            <xsl:apply-templates select="orcid"/>
        </xsl:if>
	</xsl:template>
...
<table width="100%">
<tr valign="top">
                <td width="8%">Remark type:</td>
                <xsl:if test="remark-type">
				<td width="10%">
					<xsl:value-of select="remark-type"/>
				</td>
				</xsl:if>


                <td width="6%">Remark:</td>
<!-- update for CC License on 20/4/2013   -->
