...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:variable name="ORDER">
		...
		<xsl:choose>
			<xsl:when
				test="//stage[@step='P100' or @step='P100RESUPPLY' or @step='S300' or @step='S300RESUPPLY' or @step='F300' or @step='F300RESUPPLY' or @step='Q300']">
	...
	<xsl:template match="orders">
		<html>
			...
			<body style="font-family:'Arial Unicode MS'"
				onload="expColl(part0,expanded0,document.getElementById('xmlcode'))">
				<xsl:apply-templates/>
	...
	<xsl:template match="issue-info">
		...
		<!-- issue remarks -->
		<xsl:if
			test="issue-remarks/issue-remark[remark-type!='Advert details' and remark-type!='ADV']">
	...
	<xsl:template match="*" mode="xml-source">
		...
		<div>
			<xsl:attribute name="style">
				<xsl:value-of
					select="concat('position: relative; margin-left:', $anc ,'%;', 'overflow: scroll;')"
				/>
			</xsl:attribute>
					...
					<td width="50%">
						<table width="100%">
							...
							<tr>
								...
								<td>
									<xsl:apply-templates
										select="following-sibling::s250-sequence-number"/>
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									<xsl:call-template name="insert-page-range">
										<xsl:with-param name="type">
											<xsl:value-of
												select="following-sibling::page-range/@type"/>
										</xsl:with-param>
										...
										<xsl:with-param name="first-page">
											<xsl:value-of
												select="following-sibling::page-range/first-page"/>
										</xsl:with-param>
										<xsl:with-param name="last-page">
											<xsl:value-of
												select="following-sibling::page-range/last-page"/>
										</xsl:with-param>
									</xsl:call-template>
								</td>
							</tr>
...
<xsl:text>
	...
	<xsl:template name="insert-executors">
		...
		<xsl:if
			test="$ORDER='OFFPRINTS' or $ORDER='ITEM-OFFPRINTS' or $ORDER='SEND-ITEM-OFFPRINTS' or $ORDER='PRINT-BIND'">
	...
	<xsl:template name="insert-executor">
		...
		<xsl:choose>
			...
			<xsl:when
				test="$type='PSP' or $type='ES' or $type='PRINTER' or $type='BINDER' or $type='CU-SUPPLIER'">
				<xsl:for-each select="executor[@type=$type]">
					<xsl:apply-templates select="exec-name"/>
					<xsl:if test="aff/tel">
