...
public partial class XmlOrderForm : System.Web.UI.Page
{
    ...
    protected void lnkbrowse_Click(object sender, EventArgs e)
    {
        ...
        if (txtorder.Text.Length > 0)
        {
            ...
            {
                if (Directory.Exists(orderpath + "\\" + "PPM" + "\\" + isbn_text + "\\" + DDListStage.Text + "\\Current_Order"))
                {
                    string[] PPM_Fls = null;
                    PPM_Fls = Directory.GetFiles(orderpath + "\\" + "PPM" + "\\" + isbn_text + "\\" + DDListStage.Text +"\\Current_Order");
    ...
    protected void rdppmorder_CheckedChanged(object sender, EventArgs e)
    {
        ...
        List_Folders(ddlbookid.SelectedItem.Text);
    }
    private void Open_Order(string Bookid)
    {
        ...
        if (File.Exists(orderpath + "\\" + Current_Jobtype + "\\" + isbn_text + "\\" + Current_Stage + "\\Current_Order\\" + Bookid + ".xml"))
        {
            ...
            lnkxls.Visible = true;
            lbldtd.Visible = true;
			//lnkdtd.Text = orderpath + "\\" + Current_Jobtype + "\\" + isbn_text + "\\" + Current_Stage + "\\Current_Order\\" + Bookid + ".xml";
        }
        else
        {
    ...
    private void List_Folders(string Bookid)
    {
        ...
        String tpaths=orderpath + "\\" + "PPM" + "\\" + isbn_text;
        if (Directory.Exists(tpaths ))
        {
            
            DirectoryInfo dInfo = new DirectoryInfo(tpaths);
            DirectoryInfo[] subdirs = dInfo.GetDirectories();
            for (int i = 0; i < subdirs.Length; i++)
            {
                DDListStage.Items.Add(subdirs[i].Name);            
            }
    ...
    protected void DDListStage_SelectedIndexChanged(object sender, EventArgs e)
    {
        Open_PPM_FromCombo(ddlbookid.SelectedItem.Text);
    }

    private void Open_PPM_FromCombo(string Bookid)
    {
        string stage="";
        if (DDListStage.Text == "")
        {
            txtorder.Text = "First select the stage ....";
            txtorder.ForeColor = Color.Red;
            return;
        }
        else 
        {
            stage = DDListStage.Text ;
        }
        ...
        if (Directory.Exists(orderpath + "\\" + "PPM" + "\\" + isbn_text + "\\" + stage + "\\Current_Order"))
        {
            string[] PPM_Fls = null;
            PPM_Fls = Directory.GetFiles(orderpath + "\\" + "PPM" + "\\" + isbn_text + "\\" + stage + "\\Current_Order");
            if (PPM_Fls.Length > 0)
            {
                for (int i = 0; i < PPM_Fls.Length; i++)
                {
            ...
            {
                txtorder.Text = "PPM Order is not available in the sever for " + stage  + "..";
                txtorder.ForeColor = Color.Red;
            }
        }
        else
        {
            txtorder.Text = "PPM Order is not available in the sever " + stage  + "..";
            txtorder.ForeColor = Color.Red;
        }
    }
}
