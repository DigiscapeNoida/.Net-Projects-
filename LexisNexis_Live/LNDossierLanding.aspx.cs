...
public partial class LNDossierLanding : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session[SESSION.LOGGED_USER] != null)
        {
            ...
            //// Changed by Kshitij to set "Session[SESSION.LOGGED_PRODSITE] = "DS";"

            if (string.IsNullOrWhiteSpace(Convert.ToString(Session[SESSION.LOGGED_PRODSITE])))
    ...
    protected void grdViewOrders_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "showModal")
        {
            ...
            txtfoldertitle.Text = HttpUtility.HtmlDecode(Convert.ToString(grdViewOrders.Rows[RowIndex].Cells[2].Text));
            txtAuthor.Text = HttpUtility.HtmlDecode(Convert.ToString(grdViewOrders.Rows[RowIndex].Cells[3].Text));

           // txtmailnotification.Text = HttpUtility.HtmlDecode(Convert.ToString(grdViewOrders.Rows[RowIndex].Cells[11].Text));
            Literal litimg = (Literal)gvr.FindControl("litimg");
    ...
    protected void grdViewOrders_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        foreach (GridViewRow i in grdViewOrders.Rows)
        {
            ...
            //if (Session[SESSION.LOGGED_ROLE].ToString() == "TDM" || Session[SESSION.LOGGED_ROLE].ToString() == "TDN")
            if (Session[SESSION.LOGGED_ROLE].ToString() == "TAD" || Session[SESSION.LOGGED_ROLE].ToString() == "TAD")
            {
            ...
            foreach (GridViewRow gvrow in grdViewOrders.Rows)
            {
                ...
                if (chck.Checked)
                {
                    ...
                    if (action.Equals("delete", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        query = "Update News set Active='NO', deletiondate='" + DateTime.Now.Subtract(T1) + "', STAGE = 'Deleted' where did='" + id.Trim() + "'";
                    ...
                    {
                        query = "Update News set  deletiondate='" + DateTime.Now.Subtract(T1) + "', STAGE = 'Annulé' where did='" + id.Trim() + "'";
                        int res = DataAccess.ExecuteNonQuery(query);
                        if (res > 0)
                        {
                            Sendemail("", id);
    ...
    protected void Btn_Save_Click(object sender, EventArgs e)
    {
        ...
        if (FileUpload1.HasFile)
        {
            ...
            if (ext.ToLower() == ".doc" || ext.ToLower() == ".docx")//ext == ".zip" || ext == ".rar" || ext == ".7zip" || 
            {
                ...
                try
                {
                    ...
                    if (Directory.Exists(Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString()  + "\\" + did))
                    {
                        if (!Directory.Exists(Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString()  + "\\" + did + "\\Backup"))
                        {
                            Directory.CreateDirectory(Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString()  + "\\" + did + "\\Backup");
                        }
                        string[] oldfile1 = Directory.GetFiles(Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString()  + "\\" + did + "\\Backup");

                        string[] oldfile = Directory.GetFiles(Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString()  + "\\" + did);
                        ...
                        foreach (string file in oldfile)
                        {
                            ...
                            File.Copy(file, Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString()  + "\\" + did + "\\Backup\\" + newbkpfilename, true);
                            File.Delete(file);
                        }
                    }
        ...
        if (filename == "")
        {
            query = "Update  news set CTITLE = '" + txtfoldertitle.Text.Trim() + "', Author = '" + txtAuthor.Text.Trim() + "',  Remarks = '" + txtcomment.Text.Trim() + "'  where DID = '" + Txt_ID.Text.Trim() + "'";// , mailnotification='"+txtmailnotification.Text.Trim()+"'
        }
        else
        {
            query = "Update  news set CTITLE = '" + txtfoldertitle.Text.Trim() + "', Author = '" + txtAuthor.Text.Trim() + "',  Remarks = '" + txtcomment.Text.Trim() + "',filename='" + filename + "',pagecount=null, wordcount=null,charactercount=null  where DID = '" + Txt_ID.Text.Trim() + "'";// , mailnotification='"+txtmailnotification.Text.Trim()+"'
        }
