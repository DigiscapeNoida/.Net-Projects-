    ...
    public partial class EncyclopediasLanding : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session[SESSION.LOGGED_USER] != null)
            {
                ...
                /////
                if (Session[SESSION.LOGGED_USER].ToString().Trim().ToLower().Split('@')[1] == "thomsondigital.com")
                {
        ...
        private void BindsearchGrid()
        {
            ...
            {
                for (int intRowIndex = 0; intRowIndex < grdViewOrders.Columns.Count - 6; intRowIndex++)
                {
            ...
            if (hndSelectedValue.Value.Trim().Length > 0)
            {
                ...
                foreach (SelectedData selectedData in objSelectedDataCollection.DataCollection)
                {
                    if (selectedData.SelectedValue.Count<string>() > 0)
                    {
                        ...
                        {
                            strQuery = strQuery + ((strQuery.Contains(" WHERE ") == true) ? " AND " : " WHERE ") + "CONVERT(VARCHAR(10), a." + selectedData.ColumnName + ", 105)" + " IN " + ArrayToString(selectedData.SelectedValue);// +"AND a.Active<> 'No'";
                        }
            ...
            if (hndSelectedValue.Value.Trim().Length > 0)
            {
                ...
                foreach (SelectedData selectedData in objSelectedDataCollection.DataCollection)
                {
                    if (selectedData.SelectedValue.Count<string>() > 0)
                    {
                        if (selectedData.ColumnName == "DUEDATE")
                        {
                            strQuery = strQuery + ((strQuery.Contains(" WHERE ") == true) ? " AND " : " WHERE ") + "CONVERT(VARCHAR(10), a." + selectedData.ColumnName + ", 105)" + " IN " + ArrayToString(selectedData.SelectedValue);// +"AND a.Active<> 'No' order by Articleid";
                        }
                        else if (selectedData.ColumnName == "IN_DATE")
                        {
                            strQuery = strQuery + ((strQuery.Contains(" WHERE ") == true) ? " AND " : " WHERE ") + "CONVERT(VARCHAR(10), a." + selectedData.ColumnName + ", 105)" + " IN " + ArrayToString(selectedData.SelectedValue);// +"AND a.Active<> 'No' order by Articleid";
            ...
            foreach (SelectedData selectedData in objSelectedDataCollection.DataCollection)
            {
                if (selectedData.SelectedValue.Count<string>() > 0)
                {
                    ...
                    else if (selectedData.ColumnName == "Delivery_Date")
                    {
                        if (ArrayToString(selectedData.SelectedValue) == "('')")
                        {
                            strQuery = strQuery + ((strQuery.Contains(" WHERE ") == true) ? " AND " : " WHERE ") + "a." + selectedData.ColumnName + " is null ";// +"AND a.Active<> 'No' order by Articleid";
                        }
                        else
                        {
                            strQuery = strQuery + ((strQuery.Contains(" WHERE ") == true) ? " AND " : " WHERE ") + "CONVERT(VARCHAR(10), a." + selectedData.ColumnName + ", 105)" + " IN " + ArrayToString(selectedData.SelectedValue);// +"AND a.Active<> 'No' order by Articleid";
      ...
      protected void btnExportExcel_Click(object sender, EventArgs e)
            ...
            using (StringWriter sw = new StringWriter())
            {
               ...
               // grdViewOrders.AllowPaging = false;
                ...
                foreach (GridViewRow row in grdViewOrders.Rows)
                {
                    ...
                    //row.Cells[15].Text = ggg; //As per feedbacks for missing column value
		    row.Cells[17].Text = ggg;
        ...
        protected void btnLogout_Click(object sender, EventArgs e)
        {
            ...
            */
            ...
            File.AppendAllText("C:\\Test1.txt", "1");
            ...
            File.AppendAllText("C:\\Test1.txt", "2");
                string red = "";
            try
            {
                for (int rows = 0; rows < grdViewOrders.Rows.Count; rows++)
                {
                    File.AppendAllText("C:\\Test1.txt", "a");

                    string value = grdViewOrders.Rows[rows].Cells[0].Text.ToString().Substring(0, 6);
            ...
            {
                File.AppendAllText("C:\\Test1.txt", "Exception = " + ex.ToString());
            }
            File.AppendAllText("C:\\Test1.txt", "4");
            ...
            {
                sqlparam[3] = new SqlParameter("redaction", red);
            }

            File.AppendAllText("C:\\Test1.txt", "5 = " + red);
        ...
        protected void grdViewOrders_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            foreach (GridViewRow i in grdViewOrders.Rows)
            {
                ...
                */
                ...
                //============================
                //if (Session[SESSION.LOGGED_USER].ToString().Trim().Split('@')[0].Length == 3 && Session[SESSION.LOGGED_USER].ToString().Trim().ToLower().Split('@')[1] == "lexisnexis.fr")
                //{
                    //if (lituserid.Text.Trim().Split('@')[0].Length != 3)
                    //{
                       // i.Visible = false;
                       // continue;
                    //}
                //}
                //else if (Session[SESSION.LOGGED_USER].ToString().Trim().ToLower().Split('@')[1] == "thomsondigital.com" || "Alice.Leflahec@lexisnexis.fr|Christophe.RAMPNOUX@lexisnexis.fr|fabien.rousset@lexisnexis.fr|Julie.MONTAIGNE@lexisnexis.fr".Contains(Session[SESSION.LOGGED_USER].ToString().Trim()))
                //{

                //}
                //else
                //{
                    //if (lituserid.Text.Trim().Split('@')[0].Length != 3 && lituserid.Text != Session[SESSION.LOGGED_USER].ToString())
                   // if (lituserid.Text.Trim().ToLower() != Session[SESSION.LOGGED_USER].ToString().Trim().ToLower())
                    //{
                      //  i.Visible = false;
                       // continue;
                   // }
