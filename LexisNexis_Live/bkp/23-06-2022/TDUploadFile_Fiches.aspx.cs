...
public partial class TDUploadFile1 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            // get and loda data 
            string id = Request.QueryString["FRID"].ToString();
            hiddenval.Text = id;
            SqlParameter[] paramlist = new SqlParameter[1];
            paramlist[0] = new SqlParameter("frid", id);
            DataSet ds = DataAccess.ExecuteDataSetSP(SPNames.gettduploadfiche, paramlist);
            if (ds.Tables[0].Rows.Count > 0)
            {
                txtleonardid.Text = ds.Tables[0].Rows[0]["FID"].ToString();
                txtleonardid.Enabled = false;
                txttitlefesc.Text = ds.Tables[0].Rows[0]["FTITLE"].ToString();
    ...
    protected void btnSend_Click(object sender, EventArgs e)
    {
        if (FileUpload1.HasFile)
        {
            ...
            if (ext == ".sgm" || ext == ".sgml")//ext == ".zip" || ext == ".rar" || ext == ".7zip"
            {
                ...
                try
                {
                 ...
                 //   filename = EID + "_" + hiddenval1.Text + "." + ext;
                    ...
                    paramList[0] = new SqlParameter("frid", hiddenval.Text.Trim());
                    paramList[1] = new SqlParameter("title", txttitlefesc.Text.Trim());
                    paramList[2] = new SqlParameter("tdfile", filename);
                    paramList[3] = new SqlParameter("stage", "Contenu préparé");
                    paramList[4] = new SqlParameter("InDate", Common.GetDayLightTime());

                    int rowAffected = DataAccess.ExecuteNonQuerySP(SPNames.tduploadInsertfiche, paramList);
                    if (rowAffected > 0)
                    {
                        ...
                        paramlist1[0] = new SqlParameter("fid", EID);
                        ...
                        if (set.Tables[0].Rows.Count > 0)
                        {
                           ...
                           // string strLT = Session[SESSION.LOGGED_ROLE].ToString();
                            ...
                            string strFile = Server.MapPath("App_Data\\MAILS\\THOMSON_SEND_CORRECTION\\") + "F.html";
