    ...
    public partial class EncyclopediasLanding : System.Web.UI.Page
    {
            ...
            if (hndSelectedValue.Value.Trim().Length > 0)
            {
                ...
                foreach (SelectedData selectedData in objSelectedDataCollection.DataCollection)
                {
                    if (selectedData.SelectedValue.Count<string>() > 0)
                    {
                        ...
                        {
                            strQuery = strQuery + ((strQuery.Contains(" WHERE ") == true) ? " AND " : " WHERE ") + selectedData.ColumnName + " IN " + ArrayToString(selectedData.SelectedValue);// +"AND a.Active<> 'No'";
                        }
                    }
                }
            }
      ...
      protected void btnExportExcel_Click(object sender, EventArgs e)
        ...
        protected void grdViewOrders_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "sgmlDownload")
            {

                GridViewRow row = (GridViewRow)(((LinkButton)e.CommandSource).NamingContainer);
                LinkButton lnk = (LinkButton)row.FindControl("lnksgmlfile");
                string filename = lnk.Text;
                string id = lnk.CommandArgument;
                string Expath1 = ConfigurationSettings.AppSettings["UploadFilePath"];
                string Expath = ConfigurationSettings.AppSettings["FTPFilePath"];
                if (filename != "")
                {
                    string filepath = Expath + "\\Envoi\\" + filename;
                    string path = filepath;//Server.MapPath();
                    if (File.Exists(path))
                    {
                        byte[] bts = System.IO.File.ReadAllBytes(path);
            ...
            // change on 14  march
            if (e.CommandName == "errorfile")
            {

                GridViewRow row = (GridViewRow)(((LinkButton)e.CommandSource).NamingContainer);
                LinkButton lnk = (LinkButton)row.FindControl("lnkerrorfile");
                string filename = lnk.Text;
                string id = lnk.CommandArgument;
                ...
                if (filename != "")
                {
                    string filepath = Expath + "\\Logs_LNF\\" + filename;
                    string path = filepath;//Server.MapPath();
                    if (File.Exists(path))
                    {
                        byte[] bts = System.IO.File.ReadAllBytes(path);
        ...
        protected void grdViewOrders_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            foreach (GridViewRow i in grdViewOrders.Rows)
            {
                ...
                // change on 14 march
                LinkButton lnkerrorfile = (LinkButton)i.FindControl("lnkerrorfile");
                if (cb1.ToolTip == "En correction")
                {
                    lnkerrorfile.Text = lnkerrorfile.CommandArgument.ToString().Replace(".sgml", "").Replace(".sgm", "") + "_error.txt";
                }
                // close 14 march
      ...
      protected void ddltask_SelectedIndexChanged(object sender, EventArgs e)
        ...
        protected void openButton_Click(object sender, EventArgs e)
        {
          ...
          //"</script>");
            ...
            Page.RegisterStartupScript("ScrollToBottom", "<script language='javascript'>" + System.Environment.NewLine +
         "window.location.href ='" + "http://localhost:49430/LexisNexis/FicheLanding.aspx#modal-one" + "';" + System.Environment.NewLine +
         "</script>");
        }
