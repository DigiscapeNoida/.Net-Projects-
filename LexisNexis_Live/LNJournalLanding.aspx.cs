...
public partial class LNJournalLanding : System.Web.UI.Page
{
    ...
    private void BindsearchGrid()
    {
        ...
        {
            for (int intRowIndex = 0; intRowIndex < grdViewOrders.Columns.Count - 8; intRowIndex++)
            {
    ...
    private void BindGrid()
    {
        ...
        if (queryval != "")
        {
            ...
            strQuery = "select a.*,l.firstname+' '+l.lastname as fullname,j.journal_Name from login l,journalinfo j, Article_Details a inner join (select ArticleID,max(iteration) as iteration from Article_Details group by ArticleID )b on a.ArticleID = b.ArticleID and a.iteration = b.iteration WHERE  a.userid=l.userid and a.jid=j.jid and a.JID in(" + queryval + ")";
        }
        else
        {
        ...
        if (hndSelectedValue.Value.Trim().Length > 0)
        {
            ...
            foreach (SelectedData selectedData in objSelectedDataCollection.DataCollection)
            {
                if (selectedData.SelectedValue.Count<string>() > 0)
                {
                    ...
                    {
                        if (ArrayToString(selectedData.SelectedValue) == "('')")
                        {
                            strQuery = strQuery + ((strQuery.Contains(" WHERE ") == true) ? " AND " : " WHERE ") + "cast (" + "a." + selectedData.ColumnName + " as date)" + " is null ";// +"AND a.Active<> 'No' order by Articleid";
                        }
                        else
                        {
                            strQuery = strQuery + ((strQuery.Contains(" WHERE ") == true) ? " AND " : " WHERE ") + "cast (" + "a." + selectedData.ColumnName + " as date)" + " IN " + ArrayToString(selectedData.SelectedValue);// +"AND a.Active<> 'No' order by Articleid";
 ...
 private void BindGridExcel()
    {
        ...
        if (Session["searchqryJournal"] != null)
        {
            strQuery = Session["searchqryJournal"].ToString();
        }
        else
        {
            if (queryval != "")
        ...
        //strQuery = strQuery + " AND A.Active<> 'No' order by Articleid";
		strQuery = strQuery + " AND A.In_date >= '2022-01-01' order by Articleid";
        //else
        //{
        //    strQuery = strQuery + " AND A.Active<> 'No' order by Articleid";
        //}
    ...
    protected void grdViewOrders_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        ...
        else
        {
            if (queryval != "")
            {
                ...
                "a.expire_status,a.publishing_year,a.publishing_number,a.filename,a.tdfilename,a.userid,a.articlerid,a.WorkTobeDone" +
                ",l.firstname+' '+l.lastname as fullname,j.journal_Name from login l,journalinfo j, Article_Details a inner join (select ArticleID,max(iteration) as iteration from Article_Details group by ArticleID )b on a.ArticleID = b.ArticleID and a.iteration = b.iteration WHERE  a.userid=l.userid  and a.jid=j.jid  and a.JID in(" + queryval + ")";
                //",l.firstname+' '+l.lastname as fullname,j.journal_Name from login l,journalinfo j, Article_Details a inner join (select ArticleID,max(iteration) as iteration from Article_Details group by ArticleID )b on a.ArticleID = b.ArticleID and a.iteration = b.iteration WHERE  a.userid=l.userid  and JID in(" + queryval + ")";
            }
            else
            {
                strQuery = "select  a.Articleid,a.JID,a.AID,a.Stage,a.ArticleTitle,a.ArticleType,a.Schedule_Issue,a.PIT,a.Doc_Head,a.ISSN,a.PII,a.DOI,a.TAT," +
    ...
    protected void grdViewOrders_Sorting(object sender, GridViewSortEventArgs e)
    {
        ...
        else
        {
            ...
            else
            {
               ...
               "CONVERT(VARCHAR(10), a.reviseddelivery_date, 103) + ' '  + convert(VARCHAR(8), a.reviseddelivery_date, 14) as reviseddelivery_date," +
               "a.Iteration,CONVERT(VARCHAR(10), a.Itemcomplete_date, 103) + ' '  + convert(VARCHAR(8), a.Itemcomplete_date, 14) as Itemcomplete_date," +
               "a.physicle_figs,a.bw_figs,a.Web_color_figs,a.color_figs,a.Authorname,a.email,a.status,a.no_of_loops,a.volume,a.issue,a.editor_code,a.comments,a.td_comments,a.Cancel_comment," +
               "a.expire_status,a.publishing_year,a.publishing_number,a.filename,a.tdfilename,a.userid,a.articlerid,a.WorkTobeDone" +
               ",l.firstname+' '+l.lastname as fullname,j.journal_Name from login l,journalinfo j, Article_Details a inner join (select ArticleID,max(iteration) as iteration from Article_Details group by ArticleID )b on a.ArticleID = b.ArticleID and a.iteration = b.iteration WHERE  a.userid=l.userid and a.jid=j.jid";
                //",l.firstname+' '+l.lastname as fullname from login l, Article_Details a inner join (select ArticleID,max(iteration) as iteration from Article_Details group by ArticleID )b on a.ArticleID = b.ArticleID and a.iteration = b.iteration WHERE  a.userid=l.userid";
            }
    ...
    protected void grdViewOrders_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Download")
        {
            ...
            if (filename != "")
            {
                ...
                {
                        string newPath = Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString() + "\\LN\\" + id;
                        if (Directory.Exists(newPath))
                        {
                            string[] newFileName = Directory.GetFiles(newPath);
                            bool flag = false;
                            for (int i = 0; i < newFileName.Count(); i++)
                            {
                                if (Path.GetExtension(newFileName[i].ToString()) == ".doc" || Path.GetExtension(newFileName[i].ToString()) == ".docx")
                                {
                                    path = newFileName[i].ToString();
                                    byte[] bts = System.IO.File.ReadAllBytes(path);
                                    Response.Clear();
                                    Response.ClearHeaders();
                                    Response.AddHeader("Content-Type", "Application/octet-stream");
                                    ...
                                    Response.AddHeader("Content-Disposition", "attachment;   filename=\"" + newFileName[i].ToString() + "\"");
        ...
        if (e.CommandName == "TDDownload")
        {
            ...
            if (filename != "")
            {
                ...
                {
                        string newPath = Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString() + "\\TD\\" + id;
                        if (Directory.Exists(newPath))
                        {
                            string[] newFileName = Directory.GetFiles(newPath);
                            bool flag = false;
        ...
        */
        ...
        if (e.CommandName == "showModal")
        {
            ...
            Literal ltlrid = (Literal)gvr.FindControl("ltlrid");
            txt_rid.Text = ltlrid.Text;
            Txt_ID.Text =  HttpUtility.HtmlDecode(Convert.ToString(e.CommandArgument));
            Literal ltlrid2 = (Literal)gvr.FindControl("ltlrid2");
            txt_rid2.Text = ltlrid2.Text;
            Txt_ID2.Text = HttpUtility.HtmlDecode(Convert.ToString(e.CommandArgument));
            txtarticletitle.Text = HttpUtility.HtmlDecode(Convert.ToString(grdViewOrders.Rows[RowIndex].Cells[3].Text));
              txtjournalauthor.Text = HttpUtility.HtmlDecode(Convert.ToString(grdViewOrders.Rows[RowIndex].Cells[4].Text));
              txtarticletype.Text = HttpUtility.HtmlDecode(Convert.ToString(grdViewOrders.Rows[RowIndex].Cells[5].Text));
              txtpubnum.Text = HttpUtility.HtmlDecode(Convert.ToString(grdViewOrders.Rows[RowIndex].Cells[6].Text));

            // txtmailnotification.Text = HttpUtility.HtmlDecode(Convert.ToString(grdViewOrders.Rows[RowIndex].Cells[11].Text));
              Literal litimg = (Literal)gvr.FindControl("ltlimg");

            txtcomment.Text = litimg.Text;// HttpUtility.HtmlDecode(Convert.ToString(grdViewOrders.Rows[RowIndex].Cells[12].Text));
    ...
    public void CancelSubmit(object sender, EventArgs e)
    {
        string[] str = null;
        try
        {
            string confirmValue = Request.Form["confirm_value"];
            str = confirmValue.Split('$');
            SqlParameter[] paramListemail = new SqlParameter[2];
            paramListemail[0] = new SqlParameter("Comment", str[0].Trim());
            paramListemail[1] = new SqlParameter("ArticleId", str[1].Trim());

            int rowa = DataAccess.ExecuteNonQuerySP("InsertCancelComment", paramListemail);
        }
        catch (Exception ex)
        {
            return;
        ...
        if (rowresult > 0)
        {
            ...
            if (set.Tables[0].Rows.Count > 0)
            {
                ...
                if (string.IsNullOrEmpty(set.Tables[0].Rows[0]["Email"].ToString()))
                {
                    strCC = System.Configuration.ConfigurationSettings.AppSettings["strCancelArticleCC"];
                }
                else
                    strCC = set.Tables[0].Rows[0]["Email"].ToString() + "," + System.Configuration.ConfigurationSettings.AppSettings["strCancelArticleCC"];
                // string strBCC = System.Configuration.ConfigurationSettings.AppSettings[" strCC"];
                // string strSubject = "Léonard – Demande d’intervention sur le document : "Insert Léonard id-Titre du dossier" " 
                ...
                if (File.Exists(strFile))
                {
                    ...
                    //else
                    //{
                    strBody = strBody.Replace("[IACE]", str[0].Trim());
                    //}
                    strBody = strBody.Replace("[IHT]", strLink);
        ...
        foreach (GridViewRow i in grdViewOrders.Rows)
        {
            ...
            */
            ...
            if (Session[SESSION.LOGGED_ROLE].ToString() == "LAD")
            {
                ...
                if (stageid.Text == "En préparation" && (lituserid.Text == Session[SESSION.LOGGED_USER].ToString() || Session[SESSION.LOGGED_USER].ToString() == "veema.m@thomsondigital.com" || Session[SESSION.LOGGED_USER].ToString() == "zoubaire.khodabocus@thomsondigital.com")) // En attente prod //!=|| cb1.ToolTip.ToUpper() == "ARTICLE SUBMITTED BY TD"
                {
            ...
            ImageButton img2 = (ImageButton)i.FindControl("img2");
            if (img2.CommandArgument != "" && img2.CommandArgument != null)
            {
                img2.Visible = true;
            }
            else
    ...
    protected void CheckBoxList1_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (queryval != "")
        {
            ...
            strQuery = "select a.*,l.firstname+' '+l.lastname as fullname,j.journal_Name from login l,journalinfo j, Article_Details a inner join (select ArticleID,max(iteration) as iteration from Article_Details group by ArticleID )b on a.ArticleID = b.ArticleID and a.iteration = b.iteration WHERE  a.userid=l.userid and a.jid=j.jid and a.JID in(" + queryval + ") and a.Active<> 'No' order by Articleid";
        }
        else
        {
            ...
            foreach (GridViewRow gvrow in grdViewOrders.Rows)
            {
                ...
                if (chck.Checked)
                {
                    ...
                    if (action.Equals("delete", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        query = "Update Article_Details set Active='NO', itemcomplete_date='" + DateTime.Now.Subtract(T1) + "', STAGE = 'Deleted' where ArticleID='" + id.Trim() + "'";
                    ...
                    {
                        query = "Update Article_Details set  itemcomplete_date='" + DateTime.Now.Subtract(T1) + "', STAGE = 'Annulé' where ArticleID='" + id.Trim() + "'";
                        int res = DataAccess.ExecuteNonQuery(query);
                        if (res > 0)
                        {
                            Sendemail("", id);
    ...
    protected void Btn_Save_Click(object sender, EventArgs e)
    {
            ...
            if (ext.ToLower() == ".doc" || ext.ToLower() == ".docx")//ext == ".zip" || ext == ".rar" || ext == ".7zip" || 
            {
                ...
                try
                {
                    ...
                    if (string.IsNullOrWhiteSpace(Txt_ID.Text))
                        throw new Exception();
                    string did = Txt_ID.Text.Trim();

                    //// Give path where to upload the file
                    //// First move the file to a different location

                    if (Directory.Exists(Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString() + "\\LN" + "\\" + did))
                    {
                        if (!Directory.Exists(Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString() + "\\LN" + "\\" + did + "\\Backup"))
                        {
                            Directory.CreateDirectory(Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString() + "\\LN" + "\\" + did + "\\Backup");
                        }
                        string[] oldfile1 = Directory.GetFiles(Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString() + "\\LN" + "\\" + did + "\\Backup");

                        string[] oldfile = Directory.GetFiles(Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString() + "\\LN" + "\\" + did);
                        ...
                        foreach (string file in oldfile)
                        {
                            ...
                            File.Copy(file, Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString() + "\\LN" + "\\" + did + "\\Backup\\" + newbkpfilename, true);
                            File.Delete(file);
                        }
                    }
                    ...
                    /*(string[] prevFile = Directory.GetFiles(prevFolder[0], "*");
                    if(prevFile.Count() == 0)
                        throw new Exception(" No previous file was found on the server to replace");
                    */

                    FileUpload1.SaveAs(Expath + "\\" + Session[SESSION.LOGGED_PRODSITE].ToString() + "\\LN" + "\\" + did + "\\" + filename);
        ...
        string ifUpload = (FileUpload1.HasFile) ? ", filename='" + Path.GetFileName(FileUpload1.FileName) + "'" : string.Empty;


        string query = "";
        if (filename == "")
        {
            query = "update Article_Details set ArticleTitle='" + txtarticletitle.Text + "',AuthorName='" + txtjournalauthor.Text + "',ArticleType='" + txtarticletype.Text + "',Publishing_Number='" + txtpubnum.Text + "',comments='"+txtcomment.Text +"' where ArticleRid='" + txt_rid.Text.Trim() + "'";
        }
        else
        {
            query = "update Article_Details set ArticleTitle='" + txtarticletitle.Text + "',AuthorName='" + txtjournalauthor.Text + "',ArticleType='" + txtarticletype.Text + "',Publishing_Number='" + txtpubnum.Text + "',comments='" + txtcomment.Text + "',filename='" + filename + "',pagecount=null,wordcount=null,Charactercount=null where ArticleRid='" + txt_rid.Text.Trim() + "'";
        }

     //   string query = "Update  news set CTITLE = '" + txtfoldertitle.Text.Trim() + "', Author = '" + txtAuthor.Text.Trim() + "',  comment = '" + txtcomment.Text.Trim() + "', mailnotification='" + txtmailnotification.Text.Trim() + "'  where DID = '" + Txt_ID.Text.Trim() + "'";
