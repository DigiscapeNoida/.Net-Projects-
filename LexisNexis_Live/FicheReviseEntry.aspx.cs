...
public partial class EncyclopediaReviseEntry1 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            DataSet ds = DataAccess.ExecuteDataSetSP(SPNames.getficheReviseentry, paramlist);
            txtleonardid.Text = id;
            txtleonardid.Enabled = false;
            if (ds.Tables[0].Rows.Count > 0)
            {
                txttitlefesc.Text = ds.Tables[0].Rows[ds.Tables[0].Rows.Count - 1]["FTITLE"].ToString();
            }
        ...
        if (FileUpload1.HasFile)
        {
            ...
            if (ext == ".doc" || ext == ".docx")//ext == ".zip" || ext == ".rar" || ext == ".7zip"
            {
                ...
                try
                {
                    ...
                    eridtable = DataAccess.ExecuteDataSetSP(SPNames.getfichefrid);
                    ...
                    */
                    ...
                    paramList[1] = new SqlParameter("FTITLE", txttitlefesc.Text.Trim());
                    ...
                    paramList[4] = new SqlParameter("NOTIFICATION", notifi);
                  //  paramList[9] = new SqlParameter("COMMENTS", txtcomment.Text.Trim());
                    ...
                    paramList[5] = new SqlParameter("DUEDATE", Convert.ToDateTime(dt1));
                    //  paramList[3] = new SqlParameter("REVISED_DUEDATE", "");
                    //  paramList[1] = new SqlParameter("DELIVERED_DATE", "Success");
                    paramList[6] = new SqlParameter("ITERATION", Convert.ToInt16(iteration));
                    // paramList[3] = new SqlParameter("RETURNDATE", "");
                    //  paramList[2] = new SqlParameter("PAGECOUNT", DateTime.Now);
                    paramList[7] = new SqlParameter("STAGE", "En correction");
                    paramList[8] = new SqlParameter("tat", tat);
                  //  paramList[14] = new SqlParameter("reference", reference);
                   ...
                   // paramList[16] = new SqlParameter("category", category);
                    paramList[10] = new SqlParameter("frid", ERID);
                    paramList[11] = new SqlParameter("userid", Session[SESSION.LOGGED_USER].ToString());
                    paramList[12] = new SqlParameter("InDate", Common.GetDayLightTime());
                    int rowAffected = DataAccess.ExecuteNonQuerySP(SPNames.insertrevisefiche, paramList);
                    if (rowAffected > 0)
                    {
                        lblmessage.Text = "Item ajouté";
                        //Success mail
                        SqlParameter[] paramlist11 = new SqlParameter[1];
                        paramlist11[0] = new SqlParameter("fid", EID);
                        DataSet set = DataAccess.ExecuteDataSetSP(SPNames.getemaildatafiche, paramlist11);
                        if (set.Tables[0].Rows.Count > 0)
                        {
                            ...
                            string strFile = Server.MapPath("App_Data\\MAILS\\THOMSON_RECEIVE_CORRECTION\\") + "F.html";
