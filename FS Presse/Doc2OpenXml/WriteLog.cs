...
namespace Doc2OpenXml
{
    class WriteLog
    {
        ...
        public WriteLog(string ApplicationPath)
        {
            ...
            if (!Directory.Exists(ApplicationPath + @"\LogFile\"))
            {
                Directory.CreateDirectory(ApplicationPath + @"\LogFile\");
            }
            string[] old_files = Directory.GetFiles(ApplicationPath + @"\LogFile\");
        ...
        public string GetDateLogPath()
        {
            //////////////************Start Create log file path 
            StringBuilder LogFilePath = new StringBuilder("");
            string LogDirPath = "";
           // LogDirPath = AppPath + @"\LogFile\" + DateTime.Now.AddDays(-7).ToShortDateString().Replace("/", "-"); \\anita30\12\2020
           ...
           // LogDirPath = AppPath + @"\LogFile\" + DateTime.Now.ToShortDateString().Replace("/", "-");\\anita30\12\2020

            //if (!Directory.Exists(LogDirPath)) \\anita 30\12\2020
            //{
        ...
        public void AppendLog(string LogText)
        {
           // string str=Path.GetTempPath();
            ...
            File.AppendAllText(_LogFile, DateTime.Now.ToString() +":-"+ LogText + Environment.NewLine);
