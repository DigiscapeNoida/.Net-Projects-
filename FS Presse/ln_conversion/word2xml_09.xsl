...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:output method="xml" omit-xml-declaration="no" version="1.0" encoding="iso-8859-1" doctype-public="-//EJC//DTD XML article de presse//FR" doctype-system="article.dtd" use-character-maps="SA"/>
    ...
    <xsl:template match="//auteur">
		<xsl:choose>
		    <xsl:when test="((not(following-sibling::al) or not(following-sibling::refsource)) and not(preceding-sibling::*[1][self::auteur]) and following-sibling::motscles) and (ancestor::alerte[@type eq 'veille'])">
                <motscles>
                    <xsl:copy-of select="following-sibling::motscles/@*"/>
                    <xsl:apply-templates select="following-sibling::motscles/node()"/>
                </motscles>
                <xsl:text>&#10;</xsl:text>
                <xsl:copy>
                    <xsl:apply-templates select="node()|@*"/>
    ...
    <xsl:template match="motscles">
        <xsl:choose>
            <xsl:when test="(preceding-sibling::*[1][self::auteur] or preceding-sibling::*[2][self::auteur]) and (ancestor::alerte[@type eq 'veille'])"></xsl:when>
            <xsl:otherwise>
                <xsl:copy>
                    <xsl:apply-templates select="node()|@*"/>
                </xsl:copy>
    ...
    <xsl:template match="divintro">
        <xsl:copy>
            <xsl:apply-templates select="node()|@*"/>
            <xsl:if test="following-sibling::pn">
                <xsl:copy-of select="following-sibling::pn"/>
            </xsl:if>
        </xsl:copy>
    </xsl:template>
    <xsl:template match="pn[preceding-sibling::divintro]"/>
    
    <xsl:template match="divintro/pn[1]">
        <xsl:choose>
            <xsl:when test="matches(., '[Ss][Aa][Nn][Ss][ ][Tt][Ii][Tt][Rr][Ee]')"></xsl:when>
            <xsl:otherwise>
                <xsl:element name="tit"><xsl:apply-templates/></xsl:element>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    
    <xsl:template match="corpsetu">
        <xsl:choose>
            <xsl:when test="preceding-sibling::corpsetu"></xsl:when>
            <xsl:otherwise>
                <xsl:element name="corpsetu">
                    <xsl:apply-templates select="node()|@*"/>
                    <xsl:if test="following-sibling::corpsetu">
                        <xsl:apply-templates select="following-sibling::corpsetu" mode="COR"/>
                    </xsl:if>
                </xsl:element>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="corpsetu" mode="COR">
        <xsl:apply-templates/>
    </xsl:template>
    
    <xsl:template match="div1etu[descendant::divconcl]">
        <xsl:element name="divconcl">
            <xsl:apply-templates/>
        </xsl:element>
    </xsl:template>
    
    <xsl:template match="corpsap">
        <xsl:copy>
            <xsl:apply-templates select="node()|@*"/>
        </xsl:copy>
        <xsl:if test="child::refsource[not(following-sibling::al)]">
            <xsl:copy-of select="child::refsource[not(following-sibling::al)]"/>
        </xsl:if>
    </xsl:template>
    
    <xsl:template match="refsource">
        <xsl:choose>
            <xsl:when test="parent::corpsap and not(following-sibling::al)"></xsl:when>
            <xsl:otherwise>
                <xsl:copy>
                    <xsl:apply-templates select="node()|@*"/>
                </xsl:copy>
    ...
    <xsl:template match="divconcl[ancestor::div1etu]"><xsl:apply-templates/></xsl:template>
