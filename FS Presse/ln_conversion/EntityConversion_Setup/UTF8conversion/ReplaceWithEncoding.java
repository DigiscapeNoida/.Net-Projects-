...
public class ReplaceWithEncoding {
	String inbuf="";
	byte outbuf[]=null;
	int outbufLength=0;
	...
	public boolean changeXmlFileEncodingToUTF8(String srcName, String destName){
		...
		InputStreamReader inputSreamReader;
		BufferedReader srcBufferedReader;
		...
		try {
			...
			inputSreamReader = new InputStreamReader(fis, encoding);
			srcBufferedReader = new BufferedReader(inputSreamReader);
...
//				System.out.println("LINE::> "+line);
			...
			destWriter = new OutputStreamWriter(fos, encoding);
			destWriter.write(sBuffer.toString().trim());
			destWriter.flush();			
			...
			if(fis!=null){
				try {
					...
					System.out.println("Unable to close the FileInputStream for srcFile ::> "+srcFile.getAbsolutePath());
					e.printStackTrace();
				}
			}
			if(fos!=null){
				try {
					...
					System.out.println("Unable to close the FileOutputStream for destFile ::> "+destFile.getAbsolutePath());
					e.printStackTrace();
				}
			}
	...
	public static void main(String arg[]) {
		ReplaceWithEncoding  rep = new ReplaceWithEncoding ();
		rep.process(arg[0],arg[1],arg[2]);
		rep.changeXmlFileEncodingToUTF8(arg[1],arg[1]);
