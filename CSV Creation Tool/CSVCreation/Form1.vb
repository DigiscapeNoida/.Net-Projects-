...
Public Class Form1
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        Try
            ...
            If TextBox2.Text <> "" Then
                If checkAlphabetsOnly(TextBox2.Text) Then
            ...
            If TextBox3.Text <> "" And TextBox3.Text.Contains("@") = True Then
                ...
                tempv1 = TextBox3.Text.Split(New String() {"@"}, StringSplitOptions.None)
                If tempv1.Length > 2 Then
                    Throw New Exception("Single AuthorEmail allowed..")
                End If
                AuthorEmail = TextBox3.Text
                arr1(2) = (AuthorEmail)
            Else
                Throw New Exception("Author Email must contain correct email ID..")
            End If

            If ComboBox5.Text <> "" And ComboBox5.Text.Contains("@") = True Then
                ElsevierPMMail = ComboBox5.Text.Trim
                arr1(3) = ElsevierPMMail
            Else
            ...
            If ComboBox8.Text <> "" Then
                Laung = ComboBox8.Text
                arr1(11) = (Laung)
            Else
                Throw New Exception("Laungauge must contain any value..")
            End If

            If TextBox9.Text <> "" And IsNumeric(TextBox9.Text) = True Then
                DaysforAuthor = TextBox9.Text
                arr1(8) = (DaysforAuthor)
            Else
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        Try
            ...
            xlWorkSheet.Cells(1, 4) = "ElsevierPMMail"
            xlWorkSheet.Cells(1, 5) = "ElsevierPMName"
            xlWorkSheet.Cells(1, 6) = "AdditionalCCAUMail"
            xlWorkSheet.Cells(1, 7) = "ThomsonPMMail"
            xlWorkSheet.Cells(1, 8) = "ThomsonPMname"
            xlWorkSheet.Cells(1, 9) = "DaysforAuthor"
            xlWorkSheet.Cells(1, 10) = "Type"
            xlWorkSheet.Cells(1, 11) = "FTPDetails"
            xlWorkSheet.Cells(1, 12) = "Laung"
            ...
            If File.Exists(CSVPath + "\AU_" + ISBN + ".csv") Then
                File.Delete(CSVPath + "\AU_" + ISBN + ".csv")
            End If
            xlWorkSheet.SaveAs(CSVPath + "\AU_" + ISBN + ".csv")
            xlWorkBook.Close()
            xlApp.Quit()
            ...
            fileseq = checkfileInServer(serverpath, filename)

            If fileseq = -1 Then
                MsgBox("Folder not found in server " + serverpath + ". Please use local file.")
            Else
                File.Copy(CSVPath + "\AU_" + ISBN + ".csv", serverpath & filename & "_" & fileseq.ToString & ".csv", True)
                'File.Copy(CSVPath + "\AU_" + ISBN + ".csv", "\\172.16.0.40\Elsevier English Books\Other Books\English\" & authorname & "_" & ISBN & "\Input\CSV" + "\AU_" + ISBN + "_1.csv", True)
            End If
        ...
        End Try
    End Sub
    Public Function checkfileInServer(fpath As String, fname As String) As Integer
        Dim counter As Integer = 0
        If Directory.Exists(fpath) Then
            While File.Exists(fpath + fname + "_" + (counter + 1).ToString + ".csv")
                counter = counter + 1
            End While
        Else
            Return -1
    ...
    Private Sub ElsPMMAIL_Leave(sender As Object, e As EventArgs) Handles ComboBox6.Leave
    ...
    Private Sub TDPMMAIL_Leave(sender As Object, e As EventArgs) Handles ComboBox3.Leave
        Try
            Dim hs As New Hashtable
        ...
        End Try
    End Sub
    Function checkAlphabetsOnly(intext As String) As Boolean
        Dim tempchar() As Char = intext.ToCharArray
        For a = 0 To tempchar.Length - 1
            If Char.IsLetter(tempchar(a)) Or tempchar(a) = " " Then
            Else
                Return False
            End If
        Next
